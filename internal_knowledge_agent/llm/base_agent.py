...
Adapted for Internal Knowledge Base Agent (Bank-wide).
"""
...
class IntentRouter:
    ...
    SIMPLE_PATTERNS = [
        ...
        r"tell me (about|the)",
        r"explain",
        r"what is the (process|policy|guideline)",
        r"who (is|should I)",
    ...
    COMPLEX_PATTERNS = [
        r"(create|open|submit|file).*(ticket|request|incident)",
        r"(search|find|look up).*(document|article|guide|policy)",
        r"(check|get|show).*(status|balance|info)",
        r"(troubleshoot|debug|fix|resolve)",
        r"compare.*(policies|procedures|options)",
        r"summarize.*(training|compliance|requirements)",
    ]

    CRITICAL_PATTERNS = [
        r"(delete|remove|purge).*(document|ticket|data)",
        r"(approve|reject).*(request|ticket|access)",
        r"(update|change).*(policy|procedure|rule)",
...
class Guardrails:
    ...
    TOOL_RISK_MAP: dict[str, ToolRiskLevel] = {
        ...
        "search_documents": ToolRiskLevel.SAFE,
        "get_document": ToolRiskLevel.SAFE,
        "get_document_by_version": ToolRiskLevel.SAFE,
        "list_documents": ToolRiskLevel.SAFE,
        "search_tickets": ToolRiskLevel.SAFE,
        "get_ticket": ToolRiskLevel.SAFE,
        "get_ticket_stats": ToolRiskLevel.SAFE,
        "lookup_employee": ToolRiskLevel.SAFE,
        "get_leave_balance": ToolRiskLevel.SAFE,
        "get_benefits_info": ToolRiskLevel.SAFE,
        "get_org_chart": ToolRiskLevel.SAFE,
        "check_system_status": ToolRiskLevel.SAFE,
        "search_known_issues": ToolRiskLevel.SAFE,
        "get_troubleshooting_guide": ToolRiskLevel.SAFE,
        # Low — minor side effects
        "create_ticket": ToolRiskLevel.LOW,
        "update_ticket": ToolRiskLevel.LOW,
        "send_notification": ToolRiskLevel.LOW,
        # Medium — significant side effects
        "update_document": ToolRiskLevel.MEDIUM,
        # High — irreversible, requires approval
        "delete_document": ToolRiskLevel.HIGH,
    }
...
class BaseAgent(ABC):
    ...
    def _register_rag_tools(self) -> None:
        ...
        self.tools.register(
            ...
            description="Search the internal knowledge base for products, SOPs, IT help, HR policies, compliance training, processes, and FAQ",
            parameters={
                "type": "object",
                "properties": {
                    ...
                    "collection": {
                        "type": "string",
                        "enum": ["all", "product_details", "standard_operating_procedures",
                                 "it_help_and_support", "hr_policies_and_benefits",
                                 "compliance_training", "process_guides",
                                 "regulatory_updates", "faq_and_common_questions"],
    ...
    def _build_system_prompt(self, complexity: QueryComplexity) -> str:
        base_prompt = f"""You are an Internal Knowledge Base Agent for a bank. You help employees find information about products, processes, IT help, HR policies, compliance training, and general banking knowledge.
...
2. If more detail is needed, use specific tools (ticketing, HR, ITSM)
3. Combine information from multiple sources
4. Provide a comprehensive, well-structured answer
5. Cite your sources for auditability
"""
...
High-risk tools (delete_document) require human approval.
"""
        else:
            base_prompt += """
...
"""
        ...
        base_prompt += f"""
Rules:
1. Always search knowledge base first — it contains products, SOPs, IT help, HR policies, compliance training, process guides, regulatory updates, and FAQ
2. Be helpful, professional, and concise
3. Always cite the source of your answer (department, document name)
4. If information is outdated, flag the version/date
5. If a tool fails, explain and suggest alternatives
6. Never fabricate policy or procedure information — only report what tools return
7. For IT issues, suggest troubleshooting steps and offer to create a ticket
