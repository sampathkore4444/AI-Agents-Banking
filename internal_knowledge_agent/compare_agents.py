"""
Compare all three LLM backends side by side for internal knowledge base queries.
...
TEST_QUERIES = [
    ...
    "How do I reset my VPN password? I'm having connection issues.",
...
async def run_agent(name: str, agent, query: str) -> dict:
    ...
    try:
        ...
        return {
            "agent": name, "status": "success", "answer": response.answer,
            "tools_called": [tc.name for tc in response.tool_calls],
            "tool_count": len(response.tool_calls), "duration_ms": duration,
            "tokens": response.tokens_used,
        }
    except Exception as e:
        duration = (time.time() - start) * 1000
        return {"agent": name, "status": "error", "error": str(e), "duration_ms": duration}
...
async def compare():
    ...
    print("=" * 70)
    print("Internal Knowledge Base Agent - LLM Backend Comparison")
    print("=" * 70)
    ...
    for name, agent in agents.items():
        try:
            ...
            status = "Available" if health.get("status") == "healthy" else health.get("status", "unknown")
            print(f"  {name}: {status}")
