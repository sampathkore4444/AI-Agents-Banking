...
class IntentRouter:
    ...
    SIMPLE_PATTERNS = [
        ...
        r"tell me (about|the)",
        r"explain",
        r"what (format|standard)",
    ]

    COMPLEX_PATTERNS = [
        r"(match|matching|reconcil)",
        r"(import|parse|read).*(statement|bank)",
        r"(investigate|resolve|discrepancy)",
        r"(exception|unmatched|mismatch)",
        r"(adjust|journal|ledger)",
        r"(report|summary|aging)",
        r"(embed|similarity|semantic)",
    ]

    CRITICAL_PATTERNS = [
        r"(post|create).*(adjusting|journal|entry)",
        r"(write.?off|writeoff)",
        r"(approve|authorize).*(entry|adjustment|resolution)",
        r"(reverse|void).*(entry|transaction)",
    ]
...
class Guardrails:
    ...
    TOOL_RISK_MAP: dict[str, ToolRiskLevel] = {
        ...
        "get_bank_data": ToolRiskLevel.SAFE,
        "get_ledger": ToolRiskLevel.SAFE,
        "get_account_balance": ToolRiskLevel.SAFE,
        "get_exception": ToolRiskLevel.SAFE,
        "get_exceptions": ToolRiskLevel.SAFE,
        "exception_aging": ToolRiskLevel.SAFE,
        "discrepancy_report": ToolRiskLevel.SAFE,
        "get_adjustments": ToolRiskLevel.SAFE,
        "check_gl_sync": ToolRiskLevel.SAFE,
        "parse_statement": ToolRiskLevel.SAFE,
        "match_payment": ToolRiskLevel.SAFE,
        # Low — minor side effects
        "run_auto_match": ToolRiskLevel.LOW,
        "find_discrepancies": ToolRiskLevel.LOW,
        "investigate_disc": ToolRiskLevel.LOW,
        "find_similar": ToolRiskLevel.LOW,
        "embed_inv": ToolRiskLevel.LOW,
        "embed_pay": ToolRiskLevel.LOW,
        "embed_cp": ToolRiskLevel.LOW,
        "notify_recon": ToolRiskLevel.LOW,
        # Medium — significant side effects
        "import_statement": ToolRiskLevel.MEDIUM,
        "create_recon_exception": ToolRiskLevel.MEDIUM,
        "resolve_recon_exception": ToolRiskLevel.MEDIUM,
        "escalate_recon_exception": ToolRiskLevel.MEDIUM,
        "resolve_disc": ToolRiskLevel.MEDIUM,
        "alert_exception": ToolRiskLevel.MEDIUM,
        # High — irreversible, requires approval
        "create_journal": ToolRiskLevel.HIGH,
        "reverse_ledger_entry": ToolRiskLevel.HIGH,
        "post_adjustment": ToolRiskLevel.HIGH,
    }
...
class BaseAgent(ABC):
    ...
    def _register_rag_tools(self) -> None:
        ...
        self.tools.register(
            ...
            description="Search the reconciliation knowledge base for rules, standards, and past discrepancies",
            parameters={
                "type": "object",
                "properties": {
                    ...
                    "collection": {
                        "type": "string",
                        "enum": ["all", "reconciliation_rules", "payment_standards",
                                 "exception_handling", "accounting_standards",
                                 "past_discrepancies", "matching_patterns", "regulatory_requirements"],
    ...
    async def _simple_rag_response(self, query: str) -> AgentResponse:
        ...
        answer = f"Based on our reconciliation knowledge base:\n\n" + "\n\n".join(answer_parts)
    ...
    def _build_system_prompt(self, complexity: QueryComplexity) -> str:
        ...
        base_prompt = f"""You are a Payment Reconciliation Agent for a bank. You help reconcile payments between bank statements and internal ledgers, identify discrepancies, and resolve exceptions.
...
1. Identify what data you need (bank entries, ledger entries)
2. Search knowledge base for applicable reconciliation rules
3. Use matching tools to find discrepancies
4. Investigate and resolve exceptions
5. Document all actions for audit trail
"""
        elif complexity == QueryComplexity.CRITICAL:
            base_prompt += """
This is a critical query involving high-risk actions (journal entries, reversals, write-offs).
IMPORTANT: You MUST explain your reasoning clearly before calling any tools.
These actions require human approval and SOX compliance documentation.
"""
        else:
            base_prompt += """
Be efficient. Minimize tool calls. If you can answer from retrieved context, do so.
Always follow reconciliation rules and standards from the knowledge base.
"""

        base_prompt += f"""
Rules:
1. Always search knowledge base first if unsure about reconciliation rules
2. Explain your reasoning before calling tools
3. Never fabricate information
4. Be professional and precise with financial data
5. If a tool fails, explain and suggest alternatives
6. Always maintain audit trail for compliance
