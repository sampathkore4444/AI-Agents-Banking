"""
RAG Pipeline for Customer Service & Support Agent.

Provides hybrid search over banking FAQ, product info, policies,
dispute procedures, and complaint resolution history.
"""
...
class RAGPipeline:
    ...
    def _query_collection(self, collection_name: str, query_embedding: list[float], n_results: int = 10) -> list[RetrievedChunk]:
        col = self._collections[collection_name]
        results = col.query(query_embeddings=[query_embedding], n_results=min(n_results, col.count() or 1))
        ...
        if results and results["documents"] and results["documents"][0]:
            for doc, meta, dist in zip(
                ...
                chunks.append(RetrievedChunk(text=doc, metadata=meta if meta else {}, score=1.0 - dist, collection=collection_name))
        return chunks
    ...
    def _assemble_context(self, chunks: list[RetrievedChunk]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            source = chunk.metadata.get("source", chunk.metadata.get("topic", chunk.collection))
    ...
    def query(self, user_query: str, n_results_per_collection: int = 5, top_k: int = 5, collections: list[str] | None = None) -> RAGResult:
        ...
        return RAGResult(chunks=reranked, assembled_context=self._assemble_context(reranked), query_rewrite=user_query)

    def query_single(self, collection_name: str, user_query: str, n_results: int = 5) -> list[RetrievedChunk]:
        return self._query_collection(collection_name, self._embed(user_query), n_results)
    ...
    def add_documents(self, collection_name: str, documents: list[str], metadatas: list[dict], ids: list[str]) -> None:
        ...
        self._collections[collection_name].add(documents=documents, metadatas=metadatas, ids=ids, embeddings=embeddings)
