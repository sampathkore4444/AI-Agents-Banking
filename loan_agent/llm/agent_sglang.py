...
Best for: Complex structured outputs, constrained decoding, program-like prompting.

Key advantages over vLLM:
- Native structured output (JSON schema enforcement)
- RadixAttention for prefix caching
- Compressed FSM for guided generation
...
Setup:
    ...
    3. SGLang also exposes OpenAI-compatible API
"""
...
class SGLangAgent(BaseAgent):
    """
    KYC Agent powered by SGLang.

    SGLang provides:
    - OpenAI-compatible API (we use this for simplicity)
    - Structured output with JSON schema enforcement
    - RadixAttention for fast prefix caching
    """
    ...
    async def call_llm(
        ...
        SGLang supports tool calling and structured output.
        """
    ...
    async def call_llm_structured(
        ...
        """
        Call SGLang with structured output (JSON schema enforcement).

        This is SGLang's killer feature — guaranteed valid JSON output.
        """
        payload: dict[str, Any] = {
            "model": self.model_name,
            "messages": messages,
        ...
        try:
            ...
            raise RuntimeError(f"SGLang structured output error: {e}") from e
...
# ── Structured Output Schemas for KYC ────────────────────────────

KYC_EXTRACTION_SCHEMA = {
    "type": "json_schema",
    "json_schema": {
        "name": "kyc_extraction",
        "strict": True,
        "schema": {
            ...
            "required": ["full_name", "date_of_birth", "nationality", "document_type", "confidence"],
        },
    },
}
...
RISK_ASSESSMENT_SCHEMA = {
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "required": ["risk_score", "risk_level", "flags", "recommended_action", "reasoning"],
        },
    },
}
...
TOOL_SELECTION_SCHEMA = {
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "required": ["thought", "tools_to_call", "is_final"],
        },
    },
}
