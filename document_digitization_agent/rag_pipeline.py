"""
RAG Pipeline for Document Digitization & Extraction Agent.

Provides hybrid search over document processing knowledge:
...
class RAGPipeline:
    ...
    def _rerank(self, chunks: list[RetrievedChunk], top_k: int = 5) -> list[RetrievedChunk]:
        """Simple score-based re-ranking (swap for cross-encoder in production)."""
    ...
    def _assemble_context(self, chunks: list[RetrievedChunk]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            source = chunk.metadata.get("source", chunk.metadata.get("doc_type", chunk.collection))
    ...
    def query(
        ...
        """
        Full RAG pipeline: embed → hybrid search across collections → rerank → assemble.
        """
        ...
        for col_name in target_collections:
            if col_name not in self._collections:
                continue
            all_chunks.extend(
                self._query_collection(col_name, query_embedding, n_results_per_collection)
            )
