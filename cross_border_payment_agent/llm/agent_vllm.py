...
class VLLMAgent(BaseAgent):
    def __init__(self, model_name: str = "meta-llama/Llama-3.1-8B-Instruct", vllm_url: str = "http://localhost:8000", api_key: str = "token-abc123", max_steps: int = 10, temperature: float = 0.1, max_tokens: int = 4096) -> None:
        super().__init__(model_name=model_name, max_steps=max_steps)
        self.vllm_url = vllm_url.rstrip("/")
        self.temperature = temperature
        self.max_tokens = max_tokens
        self._client = httpx.AsyncClient(base_url=self.vllm_url, headers={"Authorization": f"Bearer {api_key}"}, timeout=120.0)
    ...
    async def call_llm(self, messages: list[dict], tools: list[dict] | None = None) -> dict:
        ...
        try:
            ...
            raise RuntimeError(f"Cannot connect to vLLM at {self.vllm_url}")
        choice = data["choices"][0]
        message = choice["message"]
        tool_calls = [{"id": tc.get("id", f"vllm_{i}"), "name": tc.get("function", {}).get("name", ""), "arguments": json.loads(tc.get("function", {}).get("arguments", "{}")) if isinstance(tc.get("function", {}).get("arguments", ""), str) else tc.get("function", {}).get("arguments", {})} for i, tc in enumerate(message.get("tool_calls", []))]
...
async def main():
    ...
    if health["status"] != "healthy":
        print(f"ERROR: {health['status']}. Start: vllm serve {agent.model_name} --port 8000")
        return
