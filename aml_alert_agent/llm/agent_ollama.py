"""
Ollama Agent Backend — AML Alert Agent using Ollama.
"""
...
from base_agent import AMLAlertAgent
...
class OllamaAMLAGent(AMLAlertAgent):
...
async def main() -> None:
    ...
    print("AML Alert Agent (Ollama)")
    print(f"Model: {agent.model_name}")
    print("Commands: analyze <json>, sar <json>, decision <type> <amount> <cust_id>, history, quit\n")

    loop = asyncio.get_event_loop()
    while True:
        try:
            user_input = await loop.run_in_executor(None, lambda: input("aml> "))
        ...
        if cmd == "quit":
            ...
            args = parts[1].split()
            if len(args) >= 3:
                result = await agent.make_filing_decision(args[0], float(args[1]), args[2])
                print(json.dumps(result, indent=2))
            else:
                print("Usage: decision <activity_type> <amount> <customer_id>")
        elif cmd == "history":
            print(agent.memory.get_context())
        else:
            print("Commands: analyze <json>, decision <type> <amount> <cust_id>, history, quit")
