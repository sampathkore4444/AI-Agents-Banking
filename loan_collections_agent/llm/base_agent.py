...
class IntentRouter:
    ...
    SIMPLE_PATTERNS = [
        r"what (is|are) the (rules|requirements|policies|regulations)",
        r"how (do|does|to)",
        r"tell me (about|the)",
        r"what documents (do|are|is) (i|we|they) need",
        r"explain",
        r"what is fdcpa",
    ...
    COMPLEX_PATTERNS = [
        r"(process|create|set up|initiate).*(payment plan|collection|settlement)",
        r"recommend.*strategy|strategy.*recommend",
        r"check.*compliance|compliance.*check",
        r"(negotiate|offer).*(settlement|payment plan|hardship)",
        r"(look up|search|find).*(account|borrower|delinquent)",
        r"embed.*profile|profile.*embed",
        r"forbearance|hardship|modification|deferment",
    ]

    CRITICAL_PATTERNS = [
        r"(approve|reject|deny).*(settlement|payment plan|account)",
        r"(charge.?off|write.?off|closure).*(account)",
        r"(file.*lawsuit|legal.*action|litigation)",
        r"(repossess|foreclose|seize)",
        r"(sell|assign).*(debt|account).*third.party",
    ]
...
class Guardrails:
    ...
    TOOL_RISK_MAP: dict[str, ToolRiskLevel] = {
        ...
        "get_account": ToolRiskLevel.SAFE,
        "search_account_by_borrower": ToolRiskLevel.SAFE,
        "get_portfolio": ToolRiskLevel.SAFE,
        "get_plan": ToolRiskLevel.SAFE,
        "get_payments": ToolRiskLevel.SAFE,
        "compliance_report": ToolRiskLevel.SAFE,
        "check_program_eligibility": ToolRiskLevel.SAFE,
        "check_compliance": ToolRiskLevel.SAFE,
        "check_disclosure": ToolRiskLevel.SAFE,
        # Low — minor side effects
        "recommend_collection_strategy": ToolRiskLevel.LOW,
        "embed_profile": ToolRiskLevel.LOW,
        "log_action": ToolRiskLevel.LOW,
        "notify_borrower": ToolRiskLevel.LOW,
        "send_validation_not": ToolRiskLevel.LOW,
        "start_call": ToolRiskLevel.LOW,
        "drop_voicemail": ToolRiskLevel.LOW,
        "get_call_log": ToolRiskLevel.SAFE,
        "get_callbacks": ToolRiskLevel.SAFE,
        "call_stats": ToolRiskLevel.SAFE,
        "schedule_call": ToolRiskLevel.LOW,
        # Medium — significant side effects
        "update_account": ToolRiskLevel.MEDIUM,
        "create_plan": ToolRiskLevel.MEDIUM,
        "make_payment": ToolRiskLevel.MEDIUM,
        "offer_settlement": ToolRiskLevel.MEDIUM,
        "process_collections_payment": ToolRiskLevel.MEDIUM,
        "send_demand": ToolRiskLevel.MEDIUM,
        "enable_autopay": ToolRiskLevel.MEDIUM,
        "log_call_outcome": ToolRiskLevel.MEDIUM,
        # High — irreversible, requires approval
        "restructure_plan": ToolRiskLevel.HIGH,
    }
...
class AgentTracer:
    ...
    def get_trace_summary(self) -> dict:
        ...
        tool_calls_count = sum(1 for s in self._spans if "tool" in s.name.lower())
...
class BaseAgent(ABC):
    ...
    def _register_rag_tools(self) -> None:
        ...
        self.tools.register(
            ...
            description="Search the collections knowledge base for FDCPA regulations, strategies, and past resolutions",
            parameters={
                "type": "object",
                "properties": {
                    ...
                    "collection": {
                        "type": "string",
                        "enum": ["all", "fdcpa_regulations", "collection_strategies",
                                 "negotiation_frameworks", "compliance_guidelines",
                                 "past_resolution_cases", "hardship_programs"],
    ...
    async def _simple_rag_response(self, query: str) -> AgentResponse:
        """Handle simple queries with direct RAG retrieval."""
        ...
        answer = f"Based on our collections knowledge base:\n\n" + "\n\n".join(answer_parts)
    ...
    def _build_system_prompt(self, complexity: QueryComplexity) -> str:
        ...
        base_prompt = f"""You are a Loan Collections Agent for a bank. You help manage delinquent accounts with personalized collection strategies, payment plan negotiation, and regulatory-compliant communications.
...
- FDCPA (Fair Debt Collection Practices Act): Contact hours 8AM-9PM, max 3 attempts/day, validation notice within 5 days
- TCPA: Prior express consent for automated calls/texts
- FCRA: Accurate credit reporting, 30-day dispute investigation
- UDAAP: No unfair, deceptive, or abusive practices
"""
...
2. Search knowledge base for applicable regulations
3. Recommend or implement a strategy
4. Check compliance before any contact action
...
High-risk tools (restructure_plan) require human approval.
Always verify FDCPA compliance before taking action.
"""
        else:
            base_prompt += """
Be efficient. Minimize tool calls. If you can answer from retrieved context, do so.
Always consider compliance requirements in your responses.
"""
...
6. Always mention FDCPA compliance requirements when relevant
